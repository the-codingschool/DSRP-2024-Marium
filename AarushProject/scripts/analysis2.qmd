...
mem.maxVSize()
library(data.table)
data_distance <- as.data.table(data_distance)
```
...
# Convert distance_category to factor
...
# Set class weights
class_weights <- c(short = 2, medium = 3, long = 2)

# Train Random Forest model with class weights for distance_category
rf_model <- randomForest(distance_category ~ rideable_type + hour + day_of_week, data = train_data, ntree = 500, mtry = 2, classwt = class_weights)

# Train another Random Forest model for distance_km
rf_model2 <- randomForest(distance_km ~ rideable_type + hour + day_of_week, data = train_data, ntree = 500, mtry = 2)

# Predict on test data
predictions <- predict(rf_model, newdata = test_data)

# Convert predictions to factor with the same levels as test_data
...
sample_features <- sample_data[, c("rideable_type", "day_of_week", "hour")]

# Predict the distance
predicted_distance_km <- predict(rf_mode2l, newdata = sample_features)
...
# Check the structure of the importance data frame
print(importance_df)

# If importance is a matrix, select a specific column
if (is.matrix(importance_df)) {
  # Assuming MeanDecreaseGini column for feature importance
  importance_df <- importance_df[, "MeanDecreaseGini", drop = FALSE]
}
